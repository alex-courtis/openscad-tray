...
echo(x=x);
x_bottom = x_columns + len(columns) * t_inner + 2 * t_outer;
    ...
    color(c="darkgray") {
      translate(v=[x_bottom, 0, 0]) {
        tray(
          ...
          bottom_thickness=t_bottom,
          dividers_thickness=t_inner,
          bottom_bevel_radius=t_inner * 5,
          top_bevel_radius=t_inner * 5,
        );
      }
    }
  }
  ...
  if (render_numbers) {
    color(c="black") {
      for (i = [0:len(columns)]) {
        translate(
          v=[
            t_outer + (x_bottom - t_outer) * (columns[i] ? columns[i] : 1) - (diameters[i] + dx_diameter) / 2 - t_inner,
            y_bottom - t_inner,
            z_bottom - text_depth + 0.0001,
          ]
        ) {
          linear_extrude(height=text_depth, center=false) {
            rotate(a=90, v=[0, 0, 1]) {
              text(font=font, size=text_size, text=str(diameters[i]), halign="left", valign="center");
            }
          }
        }
      }
